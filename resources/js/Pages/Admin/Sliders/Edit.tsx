...
interface AdminSlidersEditProps extends PageProps {
    slider: Slider;
}

export default function AdminSlidersEdit({ auth, slider }: AdminSlidersEditProps) {
    const [images, setImages] = useState<ImageFile[]>(() => {
        // Only add the existing image if it has a valid URL
        if (slider.image_url) {
            return [{
                id: slider.id,
    ...
    const { data, setData, put, processing, errors, reset } = useForm<SliderForm>({
        title: slider.title || '',
        ...
        overlay_opacity: slider.overlay_opacity || 50,
        text_position: slider.text_position || 'left',
        text_alignment: slider.text_alignment || 'left',
    ...
    const [hasOriginalImage] = useState(!!slider.image_url);
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            // Create preview and update images
    ...
    const handleImagesChange = (newImages: ImageFile[]) => {
        ...
        if (newImages.length > 0) {
            ...
            // Allow removal of all images - user can upload new ones
            setData('image', null);
        }
    };

    const handleImageUpload = async (files: File[]): Promise<ImageFile[]> => {
        // For editing, we just handle local files since we'll upload on form submit
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const shouldRemoveImage = hasOriginalImage && images.length === 0 && !data.image;
        
        // Update the form data with remove_image flag
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-semantic-textSub">
                                Update slider content and styling options
                            </p>
                        </div>
                    </div>
                    <Button
